...
## - Genotyped and filtered output names must end in ".filtered.vcf.gz"
...
workflow PairedEndSingleSampleWorkflow {
  ...
  # Genotyped VCFs will take up about 98% less space than GVCFs, but maybe it's not worth making the distinction?
  ...
  output {
    ...
    File vcf_summary_metrics = CollectVcfCallingMetrics.summary_metrics
    File vcf_detail_metrics = CollectVcfCallingMetrics.detail_metrics
...
# Collect variant calling metrics from (G)VCF output
